...
+
----
$ cf api https://api.<some domain> --skip-ssl-validation
----
...
Here the CLI is creating a route with random word tokens inserted (again, see `manifest.yml` for a hint!) to prevent route collisions across the default `some.domain.com` domain.
